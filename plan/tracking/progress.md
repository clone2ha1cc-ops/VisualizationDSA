...
| **Backend .NET C#**             | 100% — Full Clean Architecture, JWT Auth, 10 Controllers, API Versioning, Caching, DB Retry, Structured Logging, Unit Tests. Optimized with LINQ Projections, AsNoTracking, and Cooperative Cancellation. |
| **Tổng file thực tế**           | ~69 files (35 frontend + 34 backend `.cs`)                         |
| **Unit tests**                  | 528 tests (522 Vitest + 6 xUnit C#) — ✅ 100% PASS                  |
...
- **JWT Authentication**: Full token-based auth với 15-minute Access Token + 30-day Refresh Token Rotation (RTR).
- **Gamification Engine**: Cộng nhận XP, thăng cấp bậc (đồng bộ bảng levels), kiểm duyệt badge và tự động cập nhật streak học tập.
- **Quiz System**: Làm bài trắc nghiệm với ngưỡng đạt 70%, tự động cộng XP thưởng.
- **Algorithm Execution API**: API thực thi giải thuật trả về frames hoạt cảnh được nén Brotli/Gzip.
- **Algorithm DoS Protection**: Giới hạn mảng đầu vào và cơ chế Timeout 2s (CancellationToken) cho giải thuật tùy biến.
- **Database Transactions**: Triển khai transaction nguyên tử (Atomicity) trong Unit of Work bọc webhook thanh toán.
- **Race Condition Prevention**: Sử dụng PostgreSQL `xmin` shadow property làm Concurrency Token chống race condition cho User.
- **Webhook Security**: Xác thực webhook SePay bằng cả API Key truyền thống và HMAC-SHA256 signature verification.
- **FluentValidation**: Tích hợp tự động kiểm tra tính hợp lệ dữ liệu đầu vào.
- **Automated Tests**: Dự án xUnit test suite (6 tests, 100% pass) phủ logic cốt lõi.
- **API Versioning (Asp.Versioning.Mvc)**: Chuẩn hóa toàn bộ route 10 Controllers sang dạng `api/v{version:apiVersion}/[controller]` và cấu hình API Versioning Explorer trong DI.
- **Memory Caching**: Cấu hình `IMemoryCache` giảm tải database và tối ưu hóa thời gian phản hồi cho Leaderboard API (top 20 users).
- **Database Indexing**: Thêm chỉ mục cột `TotalXP` trên thực thể User để tăng tốc độ sắp xếp bảng xếp hạng.
- **Database Connection Resiliency**: Cấu hình cơ chế tự động thử lại (Retry) của Npgsql trên PostgreSQL giúp hệ thống chịu lỗi mạng tốt hơn.
- **Structured Logging (Serilog)**: Middleware tự động trích xuất `UserId` và enrich vào log context cho mọi API requests để dễ dàng giám sát lỗi.
...
- ✅ **Mốc 13 (Tối ưu hóa Backend Phase 2):** DbContext Pooling, Phân trang SQL Lịch sử Quiz, và Idempotency cho Webhook thanh toán SePay.

---

## 8. ✅ Tối ưu hóa Backend Phase 2 Đã Hoàn Thành

Tất cả mục tiêu tối ưu hóa Phase 2 đã đạt:
- ✅ Cấu hình DbContext Pooling trong `Program.cs` để tái sử dụng DB context instances, tăng tốc độ phản hồi và giảm tải CPU/RAM.
- ✅ Triển khai Phân trang lịch sử Quiz dạng Skip/Take ở tầng SQL trong `QuizRepository`, tránh nạp toàn bộ danh sách vào bộ nhớ.
- ✅ Thiết lập cơ chế Idempotency cho Webhook thanh toán SePay dựa trên chỉ mục cột `TransactionReference` duy nhất của thực thể `Order` và thêm logic kiểm tra trong `PaymentService.cs`.
- ✅ Viết thêm 2 unit tests kiểm thử logic phân trang và webhook thanh toán, nâng tổng số test lên 8 tests pass 100%.
