<template>
  ...
  >
    ...
    <div 
      ...
      style="background-image: linear-gradient(to right, #1e293b 1px, transparent 1px), linear-gradient(to bottom, #1e293b 1px, transparent 1px); background-size: 3.5rem 3.5rem; mask-image: radial-gradient(ellipse 65% 55% at 50% 50%, #000 60%, transparent 100%);"
    />
    ...
    <div class="absolute top-3.5 left-4 flex flex-col gap-1 pointer-events-none">
      <span class="text-[10px] font-bold uppercase tracking-[0.08em] text-cyan-400">Algorithm Viewport</span>
      <span class="text-[13px] font-semibold text-slate-100 max-w-[340px] leading-[1.4] drop-shadow-[0_1px_8px_rgba(0,0,0,0.7)]">{{ stepDescription }}</span>
    </div>
    ...
    <div class="absolute top-3 right-3.5 flex items-center gap-2 bg-[#0e1726]/80 backdrop-blur-md border border-slate-700/70 py-1.5 px-2.5 rounded-xl">
      ...
      <!-- Zoom info -->
      <span class="text-[11px] font-mono text-cyan-400 px-1.5">{{ Math.round(camera.zoom * 100) }}%</span>
      <button class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-semibold text-slate-400 bg-transparent border border-slate-700/50 cursor-pointer transition-all duration-150 hover:text-white hover:border-slate-600 hover:bg-slate-800/40 active:scale-95" title="Reset view" @click="resetViewport">
...
import type { SortAlgorithm, SortFrame, BarStatus } from '../types/sorting.types';
...
// ─── COMPOSABLES ──────────────────────────────────────────────────────────────
const { camera, resetViewport: resetCam } = useCamera();
const { onMouseDown, onMouseMove, onMouseUp } = useMousePan(camera);
const { setupResize } = useCanvasResize(canvas, container, () => resetViewport());
...
function syncItemsToFrame(frame: SortFrame): void {
  // Cập nhật vị trí target theo arrayState
  const stateArr = frame.arrayState;
  stateArr.forEach((val, i) => {
    const item = items.value.find(it => it.value === val && Math.abs(it.targetX - i * (SLOT_W + GAP)) > 1);
    const exactItem = items.value[i] ?? items.value.find(it => it.value === val);
    if (exactItem) {
      exactItem.targetX = i * (SLOT_W + GAP);
    }
  });
  ...
  items.value.forEach((item, idx) => {
    ...
    if (isSorted)       { tgt = COLORS.sorted;  tScale = 1.0; }
    else if (isPivot)   { tgt = COLORS.pivot;   tScale = 1.08; }
    else if (isSwapped) { tgt = COLORS.swap;    tScale = 1.12; }
    else if (isCompare) { tgt = COLORS.compare; tScale = 1.05; }
    else                { tgt = COLORS.normal;  tScale = 1.0; }
...
function recompileForAlgo(algo: SortAlgorithm): void {
  ...
  const parsedArr = arrStr.split(',').map(num => parseInt(num.trim(), 10)).filter(num => !isNaN(num));
...
function render(deltaTime: number): void {
  ...
  const lerpT = Math.min(0.009 * deltaTime, 1.0);
  ...
  if (frame?.comparingIndices) {
    ...
    if (items.value[idxI]) renderLoopPointer(ctx, 'i', items.value[idxI].currentX, SLOT_W);
    if (items.value[idxJ]) renderLoopPointer(ctx, 'j', items.value[idxJ].currentX, SLOT_W, 24);
  }
