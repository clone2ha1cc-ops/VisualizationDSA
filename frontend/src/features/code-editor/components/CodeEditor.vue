  ...
  <div class="code-editor-card bg-[#0e1726]/70 backdrop-blur-md border border-slate-800/80 rounded-2xl p-6 shadow-xl flex flex-col gap-5 h-full">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-4 border-b border-slate-800">
      <div class="flex items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="text-cyan-400">
          <polyline points="16 18 22 12 16 6" />
          <polyline points="8 6 2 12 8 18" />
        </svg>
        <span class="text-xs font-bold uppercase tracking-wider text-slate-300">Soạn thảo thuật toán</span>
      </div>
    ...
    <!-- API Reference Hints -->
    <div class="bg-[#070b13] rounded-xl border border-slate-800/60 p-3.5 flex flex-col gap-2.5">
      <div class="text-[11px] font-bold uppercase tracking-widest text-slate-500">Các hàm tương tác trực quan:</div>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs font-mono">
        <div class="flex flex-col gap-0.5 bg-[#090f19] p-2 rounded-lg border border-slate-800/30">
          <span class="text-amber-400 font-bold">compare(i, j)</span>
          <span class="text-slate-400 font-sans text-[11px]">So sánh & nhấp nháy 2 phần tử</span>
        </div>
        <div class="flex flex-col gap-0.5 bg-[#090f19] p-2 rounded-lg border border-slate-800/30">
          <span class="text-rose-400 font-bold">swap(i, j)</span>
          <span class="text-slate-400 font-sans text-[11px]">Hoán vị trí hai phần tử</span>
        </div>
        <div class="flex flex-col gap-0.5 bg-[#090f19] p-2 rounded-lg border border-slate-800/30">
          <span class="text-emerald-400 font-bold">highlight(i)</span>
          <span class="text-slate-400 font-sans text-[11px]">Đánh dấu đã xếp xong</span>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 relative flex flex-col min-h-[300px]">
      <textarea
        ref="editorTextarea"
        v-model="vcrStore.code"
        @keydown.tab.prevent="handleTab"
        placeholder="// Hãy viết mã JavaScript sắp xếp của bạn ở đây..."
        class="flex-1 w-full bg-[#070b13] border border-slate-800 hover:border-slate-700 focus:border-cyan-500 rounded-xl p-4 font-mono text-sm leading-6 text-white outline-none resize-none transition-all shadow-[inset_0_2px_8px_rgba(0,0,0,0.5)] placeholder-slate-700"
      ></textarea>
    </div>
  </div>
</template>
